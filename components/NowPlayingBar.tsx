...
import { Song, PlayerState, RepeatMode } from '../types'; // CORRECTED: Path from components/ to root/
...
const NowPlayingBar: React.FC<NowPlayingBarProps> = ({ 
  ...
  const repeatColor = repeatMode !== RepeatMode.NONE ? 'text-primary' : 'text-[#CAC4D0]';
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-3 flex flex-col gap-2">
        ...
        <div className="flex items-center gap-3 w-full -mt-1 sm:mt-0">
             ...
             <div className="relative flex-1 h-8 sm:h-6 flex items-center group touch-none">
                <div className="absolute w-full h-1 bg-[#49454F] rounded-full overflow-hidden">
                    <div className="h-full bg-primary transition-all duration-100 ease-linear" style={{ width: `${(localProgress / (currentSong.duration || 1)) * 100}%` }} />
                </div>
                <input 
        ...
        <div className="flex items-center justify-between gap-3 sm:gap-4">
            ...
            <div className="flex items-center gap-1 sm:gap-4">
                <button onClick={onShuffle} className="hidden sm:block text-[#CAC4D0] hover:text-[#E6E0E9] p-2 rounded-full hover:bg-primary/10" title="Shuffle">
                    <span className="material-symbols-rounded text-xl">shuffle</span>
                </button>
                <button onClick={onPrev} className="text-[#CAC4D0] hover:text-[#E6E0E9] p-2 rounded-full hover:bg-primary/10">
                    <span className="material-symbols-rounded text-2xl sm:text-2xl">skip_previous</span>
                </button>
                
                <button 
                    onClick={onTogglePlay}
                    className="h-10 w-10 sm:h-12 sm:w-12 rounded-[12px] sm:rounded-[16px] bg-primary text-on-primary flex items-center justify-center hover:shadow-lg hover:scale-105 transition-all active:scale-95"
                >
                    <span className="material-symbols-rounded text-2xl sm:text-3xl fill-1">{isPlaying ? 'pause' : 'play_arrow'}</span>
                </button>

                <button onClick={onNext} className="text-[#CAC4D0] hover:text-[#E6E0E9] p-2 rounded-full hover:bg-primary/10">
            ...
            <div className="hidden sm:flex items-center gap-2 flex-1 justify-end">
                <div className="group flex items-center gap-2 mr-2">
                    ...
                    <input 
                        ...
                        className="w-20 h-1 bg-[#49454F] rounded-lg appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-primary"
                    />
                </div>

                <button onClick={onGoogleSearch} className="p-2 rounded-full hover:bg-primary/10 text-[#CAC4D0] hover:text-primary transition-colors" title="Search on Google">
                    <span className="material-symbols-rounded text-xl">travel_explore</span>
                </button>
                
                <button 
                    onClick={onToggleVideo} 
                    className={`p-2 rounded-full hover:bg-primary/10 transition-colors ${isVideoMode ? 'text-primary bg-primary/10' : 'text-[#CAC4D0]'}`}
                    title="Toggle Video Mode"
                >
                    <span className="material-symbols-rounded text-xl">smart_display</span>
                </button>

                <button onClick={onToggleRepeat} className={`p-2 rounded-full hover:bg-primary/10 ${repeatColor} transition-colors relative`}>
                ...
                <button onClick={onToggleLyrics} className={`p-2 rounded-full hover:bg-primary/10 transition-colors ${currentSong.lyrics ? 'text-primary' : 'text-[#CAC4D0]'}`}>
                   <span className="material-symbols-rounded text-xl">mic</span>
                </button>
            </div>
            
            <div className="sm:hidden flex items-center -mr-2">
                <button onClick={onShuffle} className="p-2 text-[#CAC4D0] hover:text-[#E6E0E9] hover:bg-primary/10 rounded-full">
                    <span className="material-symbols-rounded text-2xl">shuffle</span>
                </button>
                <button onClick={onToggleLyrics} className={`p-2 rounded-full hover:bg-primary/10 ${currentSong.lyrics ? 'text-primary' : 'text-[#CAC4D0]'}`}>
